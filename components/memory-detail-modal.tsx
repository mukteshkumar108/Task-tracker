import { Archive, Edit3, Folder, MoreVertical, Trash2, X } from "lucide-react-native";
...
export function MemoryDetailModal({
  ...
  const activeProjects = useMemo(
    () => proofProjects.filter((project) => !project.archivedAt && project.id !== liveMemory?.projectId && project.id !== liveMemory?.proofTaskId),
  ...
  const confirmArchive = () => {
    ...
    setMenuOpen(false);
    Alert.alert("Archive memory?", "This memory will be hidden from your main memories but can be restored later.", [
      { text: "Cancel", style: "cancel" },
      {
        text: "Archive",
        onPress: async () => {
          await archiveProofMemory(liveMemory.id);
          closeAll();
        },
      },
    ]);
  ...
  const confirmDelete = () => {
    ...
    setMenuOpen(false);
    Alert.alert("Delete memory?", "This proof photo will be permanently removed. This cannot be undone.", [
      { text: "Cancel", style: "cancel" },
      {
        text: "Delete Memory",
        style: "destructive",
    ...
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
      <View style={{ flex: 1 }}>
        ...
        >
          ...
          <MemoryMenuRow icon={<Archive size={18} color={colors.ink} />} label="Archive Memory" onPress={onArchive} />
